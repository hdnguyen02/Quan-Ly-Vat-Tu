...
void hienThiHoaDon(NhanVienLapHD& nvLapHD, BstVatTu& dsVatTu, int& index, NutBam& vatTu
	...
	veTableCTHD();
	OVuong(52, 180, doDaiCoban * 9 + 30 + 4, 590, 15); setcolor(CinputText);
	setbkcolor(0);
	settextstyle(6, 0, 1);
	outtextxy(margin + 1250, 720, TienichDoHoa::floatToString(nvLapHD.ptrHoaDon->getdsCTHD()->tinhTongTien()).c_str());
		...
		if (ismouseclick(WM_LBUTTONDOWN)) {
			...
			if (maNV.isMouseHover(xclick, yclick)) {
				...
				if (nhapNgay.boNhoDem != "") {
					...
					else if (nhapThang.boNhoDem != "" && nhapNam.boNhoDem != "")  {  // ki?m tra ngày tháng n?m t?ng th? ! 
				...
				if (nhapThang.boNhoDem != "") {
					...
					else if (nhapNgay.boNhoDem != "" && nhapNam.boNhoDem != "")  {  // ki?m tra ngày tháng n?m t?ng th? ! 
						tempDate.ngay = TienichDoHoa::stringToInt(nhapNgay.boNhoDem);
						tempDate.nam = TienichDoHoa::stringToInt(nhapNam.boNhoDem);
						if (!Date::ngayThangNamHopLe(tempDate.ngay,tempDate.thang,tempDate.nam)) {
							MessageBox(NULL, "Ngay-Thang-Nam khong hop le!", "Thong Bao", MB_ICONINFORMATION | MB_OK);
				...
				if (nhapNgay.boNhoDem == "" || nhapThang.boNhoDem == "" || nhapNam.boNhoDem == "" || maNV.boNhoDem == "" || soHD.boNhoDem == "") {
					...
					// tang so Luong tao HD cua nhan vien do len ! 
...
void hienThiThemHoaDonNhap(BstVatTu& dsVatTu, dsNhanVien& DSNV, int& index, NutBam& vatTu
	, NutBam& nhanVien, NutBam& doanhThu, NutBam& taoHDXuat, NutBam& inHD, NutBam& xemdsNV, NutBam& xemdsVT) {
	TienichDoHoa::xoaManHinhTheoToaDo(0, 96, DODAIMANHINH, 1000, BACKGROUP);
	NhanVien* tempNV;
	NodeHoaDon* tempHD;
	NodeCTHD* pointerCTHD;
	...
	while (true) {
		if (ismouseclick(WM_LBUTTONDOWN)) {
			...
			if (maNV.isMouseHover(xclick, yclick)) {
				...
				nhapNam.NhapVao(kiTuSo, "Chi nhap so!");
				if (nhapNam.boNhoDem != "") {
					if (TienichDoHoa::stringToInt(nhapNam.boNhoDem) == 0 || TienichDoHoa::stringToInt(nhapNam.boNhoDem) > 2022) {
						MessageBox(NULL, "Khong hop le,Thoi gian trong tuong lai!", "Thong Bao", MB_ICONINFORMATION | MB_OK);
						nhapNam.resetBoNhoDem();
					}
				}
			}
